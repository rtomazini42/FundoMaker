...
begin
   ...
   ProjectFolderPath := ExtractFilePath(ParamStr(0)); // Obtém o caminho da pasta do projeto
   OpenDialog1.InitialDir := ProjectFolderPath + 'Imgs\Background';
  {$IFDEF MSWINDOWS} // Compilação condicional para Windows
  ...
  {$ENDIF}
   {$IFDEF ANDROID}
  // Plataforma Android: Usar a função TakePhotoFromLibrary
...
begin
  fmrImage := TfmrImage.Create(Application); // Crie uma instância do formulário TImagem
  fmrImage.Logo := LogoImg;
  fmrImage.Show; // Exiba o formulário TImagem
end;
